...
| `image.tgtd.repository`            |            | tgtd image repository                                 | `kolla/ubuntu-binary-tgtd`             |
...
| `conf.lvm.lvm_target_helper`       |            | lvm target 类型 Ubuntu镜像为 tgtadm centos 镜像为 lioadm，后期优化为自动配置的 | `tgtadm`                                   |
...
| `conf.ceph.rbd_secret_uuid`        |            | ceph secret uuid                                            | `AQAcUANij7k6IhAANoVPTlE7fMY4X5draPMOqg==` |
