...
MEMCACHE_URL = os.getenv('MEMCACHE_URL', None)
...
def update_connection_fields(content):
    ...
    with open(tmp.name, 'w') as f:
        ...
        for line in content.split("\n"):
            ...
            if "database_host_placeholder" in line:
                line = line.replace("database_host_placeholder", DATABASE_URL)
            if "memcache_server_placeholder" in line:
                line = line.replace("memcache_server_placeholder", MEMCACHE_URL)
            if "rabbitmq_server_placeholder" in line:
                line = line.replace("rabbitmq_server_placeholder", RABBITMQ_URL)
            if "keystone_auth_url_placeholder" in line:
                line = line.replace("keystone_auth_url_placeholder", AUTH_URL)
            if "cinder_keystone_password_placeholder" in line:
                line = line.replace("cinder_keystone_password_placeholder", CINDER_KEYSTONE_PASSWORD)
            f.write(line + "\n")
            f.truncate()
